...
class LTServerHelper
  ...
  init: ->
    ...
    @disposables.add atom.config.onDidChange 'linter-languagetool.fallbackServer', ({newValue, oldValue}) =>
      @handlelanguagetoolServerPathSetting()
    ...
    return new Promise( (resolve, reject) =>
      @getServerInfo().then( (info) ->
        ...
        if @url is PUBLIC_LT_URL
          ...
          fallback = atom.config.get 'linter-languagetool.fallbackServer'
          if fallback?
      ...
      return new Promise( (resolve, reject) =>
        ...
        @startserver(port).then(  =>
          ...
          if fallback?
            ...
            atom.notifications.addWarning("""Unable to start the local
            server. Fallback to online server disabled.""")
        )
      )
    # Default return
    return @useLTServerWithUrl(PUBLIC_LT_URL)
